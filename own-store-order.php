...
if (isset($_REQUEST['command'])) {
	...
	header("Location: own-store-order.php");
	exit();
}

//データの表示
$sql=$pdo->prepare('SELECT * FROM order_menu join customer ON order_menu.customer_id = customer.customer_id where store_id=? ORDER BY id desc');
$sql->execute([$_SESSION['store']['id']]);
foreach ($sql as $row) {
	echo '<div class="own-order-data">';
    echo '<form class="own-order-update-form" action="own-store-order.php" method="post">';
...
<?php
	...
	echo '</div> ';
	echo '</form> ';
	echo '<form class="own-order-delete-form" action="own-store-order.php" method="post">';
	echo '<input type="hidden" name="command" value="delete">';
	echo '<input type="hidden" name="delete-id" value="', $row['id'], '">';
	echo '<input class="own-order-delete-button" type="submit" value="削除">';
	echo '</form><br>';
	echo '</div>';
	echo "\n";
}
